## 5.0.1 (March 18, 2026)
* Removed `initExternals` and `src/externals/` — `getCrypto()` now resolves crypto directly via `window.crypto` (browser) or `globalThis.crypto` (Node.js 20+ Web Crypto API), eliminating the indirection through `globalThis.passfather.externals`
